...
export default function AdminInvestmentsPage() {
  ...
  const filteredInvestments =
    filterStatus === "all" ? investments : investments.filter((inv) => inv.status === filterStatus)
      ...
      <div className="flex-1">
        ...
        <header className="bg-card border-b border-border sticky top-0 z-40 backdrop-blur-sm bg-card/95">
          <div className="px-6 py-4">
            <h1 className="text-2xl font-bold text-foreground">All Investments</h1>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-card border border-border rounded-xl p-6 hover:shadow-lg hover:shadow-primary/5 transition-all">
            ...
            <p className="text-3xl font-bold text-foreground">{filteredInvestments.length}</p>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-6 hover:shadow-lg hover:shadow-chart-2/5 transition-all">
            ...
            <p className="text-3xl font-bold text-chart-2">
              {filteredInvestments.filter((inv) => inv.status === "confirmed").length}
            </p>
          </div>
        </div>
                  ...
                  {filteredInvestments.map((investment) => (
                    <tr key={investment._id} className="hover:bg-accent/50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-foreground">
                        {investment.investorId.name}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{investment.businessId.name}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-foreground">
